...
class PreProcess:
    def __init__(self, raw_df):
        ...
        self.to_check=['distance','shortest path','distance_gate','distance_else',
                  'other_moving_ac','QDepDep','QDepArr',
                  'NDepDep','NDepArr','taxi_time']
        ...
        for i in self.to_check:
            if self.check_normality(self.processed[i]):
                self.clean_outliers_iqr(self.processed[i])
    ...
    def check_normality(self, feature, alpha=0.05):
        mu = np.mean(feature)
        std = np.std(feature)
        p = kstest(feature, cdf='norm', args=(mu, std))[1]
        return p < alpha # if ture means non-normality
...
class Analysis:
    ...
    def PCA_OnFeatures(self, acceptable=0.85, output=False, detailed=False, graph=False):
    ...
    def IteratePCA_Combinations(self, name, features, acceptable=0.85, min_feats=3, graph=False):
        ...
        self.PCA_OnFeatures(acceptable, graph=True)
    ...
    def PlotPCA_Group(self):
        ...
        for index in range(n*n):
            if index < len(self.active_features):
                ...
                fig.delaxes(ax[int(np.floor(index / n)), index % n])
        ...
        plt.savefig("./Plots/" + "-".join(self.active_features) + "PCA.png");
    ...
    def ReduceProcessedSize(self, graph=False):
        ...
        if graph:
            ...
            fig.hist(self.saved_df.loc[:, "taxi_time"], bins=250)
            fig.hist(self.saved_df.loc[final, "taxi_time"], bins=250)
            plt.xlabel("Taxi time")
            plt.ylabel("Frequency")
            plt.title("taxi time sorted distribution")
            plt.legend(["Full", "Reduced"])
            plt.savefig("./Plots/Redist.png");




def main():
    an = Analysis("features.csv")
    ...
    an.IteratePCA_Combinations("distance", distance, min_feats=4, acceptable=0.5, graph=True)
    an.IteratePCA_Combinations("congest", congestion, min_feats=4, acceptable=0.5, graph=True)
    an.ReduceProcessedSize(graph=False)
    ...
    # # Unnormalized final df
    # an.saved_df.to_csv("./final_not_normalized.csv")
